  ...
  missions: [
    {
      ...
      dataLogs: [
        {
          name: "SSEN_Traffic_Report.pdf",
          url: "https://drive.google.com/file/d/15oj1uqPtOZnMd2t_lL-hu6cYl4LdN9Td/view?usp=drive_link",
        },
      ],
      features: [
        {
    ...
    },
    {
      ...
      dataLogs: [
        {
          name: "Tutopia_Web_Traffic_Report.pdf",
          url: "https://drive.google.com/file/d/1TTcTO2jYu8uDmkEsz0dwkwQJSWmNcn6y/view?usp=drive_link",
        },
      ],
      features: [
        {
...
function WorkModeLottie({ mode }: { mode: "remote" | "onsite" | "hybrid" }) {
  ...
  const src =
    mode === "remote" ? "/images/remote_logo.json" : "/images/onsite_logo.json";
...
function CompanySection({
  ...
  const sortedMissions = [...company.missions]
    .reverse()
    .sort((a, b) =>
  ...
  const activeMission =
    sortedMissions.find((m) => m.id === activeMissionId) || sortedMissions[0];
    ...
    <div className="flex flex-col gap-6">
      ...
      >
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div
            className="flex items-center gap-2 rounded border px-3 py-1.5 font-mono text-xs"
            style={{
              borderColor:
                company.workMode === "remote"
                  ? "rgba(96,165,250,0.3)"
                  : company.workMode === "onsite"
                    ? "rgba(251,191,36,0.3)"
                    : "rgba(168,85,247,0.3)",
              color:
                company.workMode === "remote"
                ...
                >
                  ...
                  <div className="relative z-10 flex flex-col gap-7 overflow-y-auto p-5 sm:p-7">
                        ...
                        {!showLivePreview && (
                          <div className="flex items-center gap-3 rounded border border-white/8 bg-black/30 px-4 py-3">
                            ...
                            <p className="font-mono text-[0.65rem] text-white/30">
                              [ PREVIEW STANDBY — Click &quot;Show Preview&quot;
                              to load live deployment ]
                            </p>
                          </div>
                        )}
                      </div>
                    ...
                    {!activeMission.liveUrl && activeMission.videoUrl ? (
                      <div className="flex flex-col gap-3">
                        <h3 className="flex items-center gap-2 font-mono text-xs uppercase tracking-widest text-[var(--valorant-red)]">
                          <PlaySquare className="h-4 w-4 animate-pulse" /> Live
                          Video Demonstration
                        </h3>
                      ...
                      !activeMission.videoUrl &&
                      !activeMission.pdfUrl ? (
...
export default function MissionLogClient() {
  const [expandedState, setExpandedState] = useState<Record<string, boolean>>(
    Object.fromEntries(companies.map((company) => [company.name, false])),
  );
  ...
  return (
    <section className="relative mx-auto mt-12 flex max-w-7xl flex-col items-center px-4 sm:px-6">
      ...
      <div className="pointer-events-none absolute bottom-0 left-[37%] top-8 z-20 hidden w-16 flex-col items-center md:flex">
        {/* Rails */}
        <div
          className="absolute inset-y-0 left-2 w-0.5 bg-[var(--valorant-red)]/40 shadow-[0_0_8px_rgba(255,70,85,0.8)] transition-all duration-500"
          style={{
        ...
        />
        <div
          className="absolute inset-y-0 right-2 w-0.5 bg-[var(--valorant-red)]/40 shadow-[0_0_8px_rgba(255,70,85,0.8)] transition-all duration-500"
          style={{
            transform: isAllCollapsed
              ? "rotate(0deg)"
              : "rotate(3deg) translateX(20px)",
        ...
        <div
          ...
          style={{
            backgroundImage:
              "repeating-linear-gradient(to bottom, transparent, transparent 35px, rgba(255,70,85,0.4) 35px, rgba(255,70,85,0.4) 37px)",
            opacity: isAllCollapsed ? 1 : 0,
          ...
          <AnimatePresence>
            {isAllCollapsed ? (
              ...
              >
                ...
                >
                  <Lottie
                    animationData={amongUsData}
                    loop
                    autoplay
                    className="h-full w-full drop-shadow-[0_0_15px_rgba(255,255,255,0.5)]"
                  />
                </motion.div>
              </motion.div>
            ) : (
